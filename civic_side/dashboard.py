...
from graphs import (
    make_cloropleth,
    make_top_5,
    make_complaint_counts,
    make_wards_precincts,
    contributions_table,
)
import certifi
...
app.layout = html.Div(
    [
            ...
            [
                html.P(
                    [
                        ...
                        " or dive straight in and explore our findings below!",
                        html.Br(),
                    ]
                )
            ]
        ...
        dbc.Row(
            [
                html.H3(
                    "Compare how Chicago residents engage with government "
                    "across zip codes:"
                )
            ]
        ),
            ...
            [
                dbc.Col(
                    [
                        dcc.Graph(
                            id="cloropleth1",
                            figure=make_cloropleth(
                                df, df_revised, zipcodes, "blues", "votingrates"
                            ),
                        ),
                        html.H6(
                            "Note: 60612 contains the largest conglomeration of hospitals downtown",
                            style={
                ...
                dbc.Col(
                    dcc.Graph(
                        id="cloropleth2",
                        figure=make_cloropleth(
                            df, df_revised, zipcodes, "greens", "avg_donation"
                        ),
                    )
                ),
            ]
        ...
        dbc.Row(
            [
                html.P(
                    "Tables default to city-wide statistics."
        ...
        dbc.Row(
            [
                dcc.Dropdown(
                    ["City of Chicago"] + list(df["zip"].unique()),
                    "City of Chicago",
                    id="dropdown",
                )
            ]
        ),
        dbc.Row(dcc.Graph(id="total complaints", figure=make_complaint_counts(df))),
        dbc.Row(
            [
                dbc.Col(dcc.Graph(id="top5", figure=make_top_5())),
                dbc.Col(dcc.Graph(id="wards", figure=make_wards_precincts())),
            ]
        ),
        dbc.Row(dcc.Graph(id="campaigns", figure=contributions_table())),
    ],
    style={
        "margin-left": "60px",
        "width": "90%",
...
def update_table(value):
    ...
    if value != "City of Chicago":
        return (
            make_complaint_counts(df, value),
            make_top_5(value),
            contributions_table(value),
            make_wards_precincts(value),
        )
    return (
        make_complaint_counts(df),
        make_top_5(),
        contributions_table(),
        make_wards_precincts(),
    )
