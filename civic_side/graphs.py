...
def make_cloropleth(df, df_revised, zipcodes, color, default):
    ...
    if default == "votingrates":
        ...
        button2 = dict(
            method="restyle",
            args=[{"z": [df["votingrates"]], "tickformat": ",.0%"}],
            label="Voting Rates (Percentages)",
        )
        button1 = dict(
            method="restyle",
...
def make_top_5(zipcode=None):
    ...
    """
    top5 = pd.read_csv(
        pathlib.Path(__file__).parent / "the_polis/311_topcomplaints_byzip.csv"
    )
    if zipcode == None:
        data = (
            top5.groupby("SR_TYPE")
            .sum()
            .sort_values(by="complaintcounts", ascending=False)
            .iloc[0:5]
            .reset_index()
        )
    else:
    ...
    complaints = go.Figure(
        go.Bar(
            x=list(data["SR_TYPE"]), y=list(data["complaintcounts"]), orientation="v"
        )
    )
...
def make_complaint_counts(df, zipcode=None):
    ...
    complaints_count = go.Figure(
        data=[
            go.Table(
                header=dict(
                    values=["2019 311 Calls"],
                    line_color="white",
...
def make_wards_precincts(zipcode=None):
    ...
    """
    wards = pd.read_csv(
        pathlib.Path(__file__).parent / "the_polis/clean_zipcode_precinct.csv"
    )
    ...
    wards = go.Figure(
        data=[
            go.Table(
                ...
                cells=dict(
                    values=[data["ward"], data["precinct"]],
    ...
    wards.update_layout(
        height=int(400), title="What are the Voting Districts represented?"
    )
    return wards
...
def contributions_table(zipcode=None):
    ...
    contributions.update_layout(
        height=int(400), title="2019 Mayoral Campaign Contributions"
    )
