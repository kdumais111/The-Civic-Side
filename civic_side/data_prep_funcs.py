...
def prep_all_data():
    ...
    zillow = pathlib.Path(__file__).parent / "the_polis/zillow_cleaned_complete.csv"
    voting_housing = combine_data_zip(voting_zipcode, zillow, "Zipcode")
    complaints = pathlib.Path(__file__).parent / "the_polis/311_complaint_count.csv"
...
def voting_to_zipcode():
    ...
    """
    districts = pathlib.Path(__file__).parent / "the_polis/clean_zipcode_precinct.csv"
    ...
    voting_district = pd.merge(
        districts_df, voting_df, on=["ward", "precinct"], how="inner"
    )
    voting_combined = voting_district.groupby(["zip"]).sum().reset_index()
    voting_combined["votingrates"] = (
        voting_district["Ballots Cast"] * 100 / voting_district["Register Voters"]
    )
...
def combine_data_zip(df, filename, column):
    ...
    if column == "zip":
        ...
        merged_set = pd.merge(
            df, new_data, left_on="zip", right_on=column, how="left"
        ).drop(columns=[column])
